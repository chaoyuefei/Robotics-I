...
class MinimalClient(Node):
    ...
    def send_request(self):
        ...
        self.req.goal_angularposition = 0.34
        #self.req.goal_linearposition = 100.
        #self.req.goal_effort = 10.
        #self.req.goal_velocity = 30.
...
def main(args=None):
    ...
    if node.future.result() is not None:
        node.get_logger().info('Goal accepted: %d: ' %
                               node.future.result().goal_accepted)
    else:
        node.get_logger().error('Exception while calling service: %r' %
                                node.future.exception())
