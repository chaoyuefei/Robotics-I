...
int main(int argc, char *argv[]) {
  ...
  // "/hrim_actuation_servomotor_000000000001/goal_axis1"
  ...
  auto pub2 = node->create_publisher<
      hrim_actuator_rotaryservo_msgs::msg::GoalRotaryServo>(
  ...
  // Create message with the same type as the topic,
  // <hrim_actuator_rotaryservo_msgs::msg::GoalRotaryServo>.
  auto msg =
      std::make_shared<hrim_actuator_rotaryservo_msgs::msg::GoalRotaryServo>();
  auto msg2 =
      std::make_shared<hrim_actuator_rotaryservo_msgs::msg::GoalRotaryServo>();

  // Desired position in degrees
  float position_deg = 30;
  float position_deg2 = 30;
  ...
  while (rclcpp::ok()) {
    ...
    msg2->position = position_deg2 * 3.1416 / 180.0; // Position to rads
